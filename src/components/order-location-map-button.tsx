...
export function OrderLocationMapButton({ title, address, longitude, latitude }: Props) {
    ...
    async function initMap() {
      ...
      try {
        ...
        const info = new window.AMap.InfoWindow({
          content: `${title || "单据"}<br/>${address || "未填写地址"}<br/>${longitude.toFixed(6)}, ${latitude.toFixed(6)}`,
  ...
  }, [address, canShow, latitude, longitude, open, title]);
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/60 p-3">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-4 shadow-2xl ring-1 ring-slate-100">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900">单据位置</h3>
              <button
                type="button"
