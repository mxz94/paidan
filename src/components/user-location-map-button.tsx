...
export function UserLocationMapButton({ username, displayName, longitude, latitude }: Props) {
  const [open, setOpen] = useState(false);
  const [errorText, setErrorText] = useState("");
  const mapContainerRef = useRef<HTMLDivElement | null>(null);
  const mapRef = useRef<any>(null);

  const canShow = longitude != null && latitude != null;
    ...
    async function initMap() {
      ...
      try {
        ...
        const info = new window.AMap.InfoWindow({
          content: `${displayName} (${username})<br/>${longitude.toFixed(6)}, ${latitude.toFixed(6)}`,
  ...
  }, [canShow, displayName, latitude, longitude, open, username]);
  ...
  return (
    <>
      <button
        ...
        className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-slate-300 text-slate-700 disabled:cursor-not-allowed disabled:opacity-40"
        title={canShow ? "查看位置" : "暂无位置"}
      >
      ...
      {open ? (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/60 p-3">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-4 shadow-2xl ring-1 ring-slate-100">
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-base font-bold text-slate-900">用户位置</h3>
              <button
                type="button"
